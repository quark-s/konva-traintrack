import { postLogEvent } from './log.mjs';
import {
    TrackType1,
    TrackType2,
    TrackType3,
...
var TStage = (function () {
    ...
    function addTrack(d){    
        try {
            ...
            let tmp = eval(`new ${d.type}(${JSON.stringify(d.pos)}, ${factor*parseFloat(d.width)}, ${factor*parseFloat(d.height)}, ${d.rotation})`);
    ...
    let triangleFrame = new Konva.RegularPolygon({
        x: 700,
        y: 200,
        sides: 3,
        radius: config.unitSize*5.2+5,
        // fill: '#EEE',
        stroke: '#b6eddb',
        // dash: [2,2],
        strokeWidth: 5,
        name: 'slot-triangle-frame',
        shadowColor: 'black',
        shadowBlur: 2,
    ...
    layer.add(triangle, triangleFrame);
    ...
    let circleFrame = new Konva.Circle({
        x: 420,
        y: 170,
        radius: config.unitSize*4.5+5,
        // fill: '#EEE',
        stroke: '#b6eddb',
        // dash: [2,2],
        strokeWidth: 5,
        name: 'slot-circle-frame',
        shadowColor: 'black',
        shadowBlur: 2,
        shadowOffset: { x: 2, y: 2 },
        shadowOpacity: 0.5,        
