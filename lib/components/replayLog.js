...
class TraintrackReplayLog extends LitElement {
    ...
    renderAction(action, i){
        ...
        let _labels = action?.data?.labels ? html`<div><strong>labels: </strong> ${JSON.stringify(action?.data?.labels)}</div>` : "";
        let _rotation = action?.type == "rotate" && action?.data?.rotation ? html`<div><strong>rotation: </strong> ${action?.data?.rotation}</div>` : "";
        return html`
            <div class="${_class}">
                <div><strong>type: </strong> ${action?.type}</div>
                ${_labels}
                ${_rotation}
                <div title="hh:mm:ss.ms"><strong>relative time: </strong> ${_relativeTime.hours().toString().padStart(2, "0")}:${_relativeTime.minutes().toString().padStart(2, "0")}:${_relativeTime.seconds().toString().padStart(2, "0")}.${_relativeTime.milliseconds()}</div>
