...
export const LogHistory = (function () {
    ...
    return {
        pushLog: function (data) {
            ...
            logs.push(data);
            if(typeof onAfterPush == "function")
                onAfterPush(data);
        },
        getLogs: (complete, stringify) => {

            if(typeof stringify !== "boolean")
                stringify = true;

            if (!!complete)
                return stringify ? JSON.stringify(logs) : logs;
            
            let tmp = logs.map(event => event?.stagedata ? { ...event, stagedata: "placeholder-for-stagedata" } : event);
            return stringify ? JSON.stringify(tmp) : tmp;
        },
        downloadLogs: () => {
        ...
        setOnAfterPush(callback){
            if(typeof callback == "function")
                onAfterPush = callback;
        }
    };
